function [ out, props ] = predictImage( model, features, classNames)
...
w = whos('oneRow');
bigDataLimit = round((16*2^20)/w.bytes); % This should create 16 MB batches of the data
...
if featureSize<=bigDataLimit        
    ...
    uicontrol('Parent',infoD,'Style','text','Units','Pixels','Position',[30 20 240 40],'String','Classifing cells based on your training... (Prediction) This will not take long.');    
        ...
        trainingData.instances = features;
        trainingData.labels = ones(size(features, 1),1);
...
%             out(out<0) = 2;
%         else
            [out,props] = sacPredict(model, trainingData);
%         end
